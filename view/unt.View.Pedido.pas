...
*                                                                             *
* Candidato: Rawgleison Batista Amaral                                        *
* Data: 14/03/2023                                                            *
*                                                                             *
*                                                                             *
* Projeto consiste em uma proglama de pedido, onde é apresentado cadastro de  *
* cliente, produtos e usuários, contendo um sistema                           *
...
uses
  ...
  unt.View.Users, unt.Model.DTO.User;
...
begin
  if Message_ATENCAO_Confirma('Deseja limpara a tela?'#13'Todo o progresso não salvo será perdido.') then
    Status := stAGUARDANDO;
end;

procedure TfrmPedido.butPedExcluirClick(Sender: TObject);
begin
  if Message_ATENCAO_Confirma
    ('Deseja excluir o pedido permanentemente?'#13 +
    'Esta ação não poderá ser desfeita.') then
  begin
...
var
  itemMsg: String;
begin
  itemMsg := memTblItens.FieldByName('item').AsString + ' - ' +
    memTblItens.FieldByName('descricao').AsString;
...
begin
  Status := stAGUARDANDO;
  Usurios1.Visible := FUserLogado.nivel = tpADMIN;
  Self.Caption := 'Pedidos Teste - Usuário: '+FUserLogado.nome;
end;
...
begin
  ...
  case FStatus of
    ...
    stINSERINDO_PEDIDO:
      memTblItens.Open;
    stINSERINDO_PRODUTO:
      edtProdVrUnit.SetFocus;
  end;

end;

procedure TfrmPedido.Usurios1Click(Sender: TObject);
begin
  frmCadastroUser.ShowModal;
end;
