...
procedure TfrmCadastroProduto.btnCliExcluirClick(Sender: TObject);
begin
  if Message_ATENCAO_Confirma('Deseja deletar o produto '+edtDescricao.Text+' permanentemente?') then
  begin
    FProdutoCtrl.Delete(StrToInt(edtCod.Text));
    Status := stAGUARDANDO;
  end;
end;
...
procedure TfrmCadastroProduto.edtCodKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
begin
  ...
  begin
    Message_ATENCAO('Produto não localizado na base de dados');
    Status := stAGUARDANDO;
    Exit;
  end;
